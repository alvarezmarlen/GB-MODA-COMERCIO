  ...
  <DashboardTemplate :title="t('admin.title')">
      ...
      <div v-if="stories.length > 0" class="admin-stories-list">
        <div v-for="story in stories" :key="story.id" class="admin-story-card">
          ...
          <div class="admin-card-actions">
            <button class="wireframe-button edit-btn" @click="startEdit(story)">{{ t('admin.modify') }}</button>
            <button class="wireframe-button delete-btn" @click="confirmDelete(story)">{{ t('admin.delete') }}</button>
          </div>
        </div>
      </div>
      ...
      <div class="wireframe-container modal-content">
        ...
        <div class="modal-fields">
          <div class="modal-field">
            <label class="wireframe-label">{{ t('admin.titleLabel') }}</label>
            <input v-model="editForm.title" class="wireframe-input" type="text" />
          </div>
          <div class="modal-field">
            <label class="wireframe-label">{{ t('admin.descriptionLabel') }}</label>
            <textarea v-model="editForm.content" class="wireframe-input modal-textarea" rows="4"></textarea>
          </div>
        </div>
        <div class="modal-actions">
          <button class="wireframe-button" @click="cancelEdit">{{ t('admin.cancel') }}</button>
          <button class="wireframe-button save-btn" @click="saveEdit">{{ t('admin.save') }}</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="deletingStory" class="modal-overlay" @click.self="cancelDelete">
      <div class="wireframe-container modal-content modal-small">
        ...
        <div class="modal-actions">
          <button class="wireframe-button" @click="cancelDelete">{{ t('admin.cancel') }}</button>
          <button class="wireframe-button delete-btn" @click="executeDelete">{{ t('admin.delete') }}</button>
        </div>
      </div>
    </div>
  </DashboardTemplate>
